/**
 * NAVADA CardioAtlas — interactive segmented heart.
 *
 * Sister page to the brain atlas. Reuses the platform's shared modules
 * (scene, camera, lighting, interaction, explode) and adds heart-specific
 * managers: cardiac-state heatmap + live heartbeat (HeartActivity) and an
 * ECG / pressure telemetry dashboard (HeartTelemetry).
 */
...
const CHAMBER_IDS = new Set(['Right atrium', 'Right ventricle', 'Left atrium', 'Left ventricle']);
...
class NavadaHeart {
    ...
    async init() {
        try {
            this.regions = await fetch('data/heart-regions.json').then(r => r.json());
            for (const key in this.regions) this.normToKey.set(norm(key), key);

            this.heartModel = new OrganModel(this.sceneManager.scene);
            await this.heartModel.load('models/heart.glb');
            ...
            this.activity = new HeartActivity(this.sceneManager.scene, this.idToMesh, this.regions);

            this.telemetry = new HeartTelemetry(
                ...
                { status: document.getElementById('tele-status'),
                  hr: document.getElementById('tele-hr'),
                  bp: document.getElementById('tele-bp'),
    ...
    _prepareMeshes() {
        this.heartModel.model.traverse((o) => {
            if (!o.isMesh) return;
            ...
            // paint each part its region colour so the resting heart is colour-coded
            // (blue right side, red left side, distinct valves) — like the brain atlas
    ...
    _addHelpers() {
        ...
        this.grid = new THREE.GridHelper(7, 14, 0xff5a6e, 0x402028);
    ...
    _frameModel() {
        // same 3/4 framing as the brain atlas
        const sphere = this.heartModel.getBoundingSphere();
        const dist = this.heartModel.getOptimalCameraDistance(this.sceneManager.camera.fov);
        this.camera.camera.position.set(dist * 0.55, dist * 0.22, dist * 0.82);
        this.camera.setTarget(this.heartModel.getCenter());
    ...
    select(id) {
        ...
        this.leaderColor = r.color || '#ff5a6e';
    ...
    // X-ray: fade the chamber walls so the valves & papillary muscles inside show
    setReveal(on) {
        for (const [id, mesh] of this.idToMesh)
            if (CHAMBER_IDS.has(id)) mesh.material.opacity = on ? 0.14 : 1;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    const app = new NavadaHeart();
    app.init();
    window.navadaHeart = app;
});
