/**
 * NAVADA RespiroAtlas — interactive segmented lungs.
 *
 * Third organ in the NAVADA Atlas. Reuses the shared platform modules and adds
 * lung-specific managers: ventilation heatmap + live breathing (LungActivity)
 * and a spirogram / ventilation telemetry dashboard (LungTelemetry).
 */
...
import { LungActivity, SCENARIOS } from './modules/LungActivity.js';
import { LungTelemetry } from './modules/LungTelemetry.js';
import { TutorManager } from './modules/TutorManager.js';

const LOBE_IDS = new Set([
...
class NavadaLungs {
    ...
    async init() {
        try {
            this.regions = await fetch('data/lungs-regions.json').then(r => r.json());
            for (const key in this.regions) this.normToKey.set(norm(key), key);

            this.lungModel = new OrganModel(this.sceneManager.scene);
            await this.lungModel.load('models/lungs.glb');
            ...
            this.activity = new LungActivity(this.sceneManager.scene, this.idToMesh, this.regions);

            this.telemetry = new LungTelemetry(
                ...
                { status: document.getElementById('tele-status'),
                  rr: document.getElementById('tele-rr'),
                  spo2: document.getElementById('tele-spo2'),
                  tidal: document.getElementById('tele-tidal') },
                this.activity);
            ...
            this._modelBaseScale = this.lungModel.model.scale.clone();
            this._buildUI();
            this.loadingEl.classList.add('hidden');
            this.animate();
            console.log('NAVADA RespiroAtlas ready —', this.meshes.length, 'parts');
        } catch (err) {
            console.error(err);
    ...
    _prepareMeshes() {
        this.lungModel.model.traverse((o) => {
            if (!o.isMesh) return;
    ...
    _addHelpers() {
        ...
        this.grid = new THREE.GridHelper(7, 14, 0x34d399, 0x1e4035);
    ...
    _frameModel() {
        // same 3/4 framing as the brain & heart atlases
        const sphere = this.lungModel.getBoundingSphere();
        const dist = this.lungModel.getOptimalCameraDistance(this.sceneManager.camera.fov);
        this.camera.camera.position.set(dist * 0.55, dist * 0.22, dist * 0.82);
        this.camera.setTarget(this.lungModel.getCenter());
    ...
    select(id) {
        ...
        this.leaderColor = r.color || '#34d399';
    ...
    // X-ray: fade the lobes so the trachea, bronchi & diaphragm inside show
    setReveal(on) {
        for (const [id, mesh] of this.idToMesh)
            if (LOBE_IDS.has(id)) mesh.material.opacity = on ? 0.16 : 1;
    }
    ...
    animate() {
        ...
        // make the lungs visibly breathe with the respiratory cycle
        if (this._modelBaseScale && this.activity) {
            const s = 1 + this.activity.inflation * 0.06;
            this.lungModel.model.scale.set(
                this._modelBaseScale.x * s, this._modelBaseScale.y * (s * 1.02), this._modelBaseScale.z * s);
        }
        this._updateLeader();
...
document.addEventListener('DOMContentLoaded', () => {
    const app = new NavadaLungs();
    app.init();
    window.navadaLungs = app;
});
