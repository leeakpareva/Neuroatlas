/**
 * NAVADA GutAtlas — interactive large intestine (colon + appendix).
 *
 * Seventh organ in the NAVADA Atlas. Reuses the shared platform modules and
 * adds gut-specific managers: a transit heatmap + live bowel behaviour
 * (GutActivity — motility, hydration, transit) and a motility / colon-transit
 * telemetry dashboard (GutTelemetry). A peristaltic wave travels down the colon
 * segments (duodenum -> ascending -> transverse -> descending -> sigmoid).
 */
...
import { GutActivity, SCENARIOS } from './modules/GutActivity.js';
import { GutTelemetry } from './modules/GutTelemetry.js';
import { TutorManager } from './modules/TutorManager.js';

// the colon segments — faded by X-ray to spotlight the small appendix
const COLON_IDS = new Set(['Ascending colon', 'Transverse colon', 'Descending colon', 'Sigmoid colon']);
// peristaltic-wave order (the wave travels down this sequence)
const PERI_ORDER = ['Duodenum', 'Ascending colon', 'Transverse colon', 'Descending colon', 'Sigmoid colon'];
...
class NavadaGut {
    ...
    async init() {
        try {
            this.regions = await fetch('data/gut-regions.json').then(r => r.json());
            for (const key in this.regions) this.normToKey.set(norm(key), key);

            this.gutModel = new OrganModel(this.sceneManager.scene);
            await this.gutModel.load('models/gut.glb');
            ...
            this.activity = new GutActivity(this.sceneManager.scene, this.idToMesh, this.regions);

            this.telemetry = new GutTelemetry(
                ...
                { status: document.getElementById('tele-status'),
                  motility: document.getElementById('tele-motility'),
                  transit: document.getElementById('tele-transit'),
            ...
            this.animate();
            console.log('NAVADA GutAtlas ready —', this.meshes.length, 'parts');
        } catch (err) {
            console.error(err);
    ...
    _prepareMeshes() {
        this.gutModel.model.traverse((o) => {
            if (!o.isMesh) return;
        ...
        // peristaltic wave travels down the tract in order
        PERI_ORDER.forEach((id, i) => {
            const mesh = this.idToMesh.get(id);
            if (mesh) this.periParts.push({ mesh, baseScale: mesh.scale.clone(), phase: i * 1.1 });
        });
    }
    ...
    _addHelpers() {
        ...
        this.grid = new THREE.GridHelper(7, 14, 0xf97316, 0x3a2410);
    ...
    _frameModel() {
        const sphere = this.gutModel.getBoundingSphere();
        const dist = this.gutModel.getOptimalCameraDistance(this.sceneManager.camera.fov);
        this.camera.camera.position.set(dist * 0.55, dist * 0.22, dist * 0.82);
        this.camera.setTarget(this.gutModel.getCenter());
    ...
    select(id) {
        ...
        this.leaderColor = r.color || '#f97316';
    ...
    // X-ray: fade the colon so the small appendix stands out
    setReveal(on) {
        for (const [id, mesh] of this.idToMesh)
            if (COLON_IDS.has(id)) mesh.material.opacity = on ? 0.18 : 1;
    }
    ...
    animate() {
        ...
        // a peristaltic wave travels down the tract, scaled by motility
        if (this.periParts.length && this.activity) {
            const m = this.activity.motility;
            for (const { mesh, baseScale, phase } of this.periParts) {
                const pulse = 1 + m * 0.05 * Math.sin(this._tAcc * 2.4 - phase);
...
document.addEventListener('DOMContentLoaded', () => {
    const app = new NavadaGut();
    app.init();
    window.navadaGut = app;
});
