/**
 * NAVADA GlucoAtlas — interactive metabolic organs (stomach, pancreas, gut).
 *
 * Sixth organ in the NAVADA Atlas. Reuses the shared platform modules and adds
 * metabolic managers: a glucose-state heatmap + live blood-sugar behaviour
 * (GlucoActivity — glucose, insulin, glucagon, GLP-1, peristalsis) and a
 * continuous-glucose / hormone telemetry dashboard (GlucoTelemetry). The
 * stomach & duodenum ripple with digestion and the pancreas pulses as it
 * releases insulin.
 */
...
import { GlucoActivity, SCENARIOS } from './modules/GlucoActivity.js';
import { GlucoTelemetry } from './modules/GlucoTelemetry.js';
import { TutorManager } from './modules/TutorManager.js';

// anterior organs faded by X-ray to reveal the pancreas behind them
const FRONT_IDS = new Set(['Liver', 'Stomach']);
// hollow gut organs that ripple with digestion (peristalsis)
const PERI_ORDER = ['Stomach', 'Duodenum'];
...
class NavadaGluco {
    ...
    async init() {
        try {
            this.regions = await fetch('data/gluco-regions.json').then(r => r.json());
            for (const key in this.regions) this.normToKey.set(norm(key), key);

            this.glucoModel = new OrganModel(this.sceneManager.scene);
            await this.glucoModel.load('models/gluco.glb');
            ...
            this.activity = new GlucoActivity(this.sceneManager.scene, this.idToMesh, this.regions);

            this.telemetry = new GlucoTelemetry(
                ...
                { status: document.getElementById('tele-status'),
                  glucose: document.getElementById('tele-glucose'),
                  insulin: document.getElementById('tele-insulin'),
            ...
            this.animate();
            console.log('NAVADA GlucoAtlas ready —', this.meshes.length, 'parts');
        } catch (err) {
            console.error(err);
    ...
    _prepareMeshes() {
        this.glucoModel.model.traverse((o) => {
            if (!o.isMesh) return;
        ...
        // digestion ripple on the hollow gut + the pancreas insulin pulse
        PERI_ORDER.forEach((id, i) => {
            const mesh = this.idToMesh.get(id);
            if (mesh) this.periParts.push({ mesh, baseScale: mesh.scale.clone(), phase: i * 1.4 });
        });
        const panc = this.idToMesh.get('Pancreas');
        if (panc) this.pancreas = { mesh: panc, baseScale: panc.scale.clone() };
    }
    ...
    _addHelpers() {
        ...
        this.grid = new THREE.GridHelper(7, 14, 0xec4899, 0x3a1030);
    ...
    _frameModel() {
        const sphere = this.glucoModel.getBoundingSphere();
        const dist = this.glucoModel.getOptimalCameraDistance(this.sceneManager.camera.fov);
        this.camera.camera.position.set(dist * 0.55, dist * 0.22, dist * 0.82);
        this.camera.setTarget(this.glucoModel.getCenter());
    ...
    select(id) {
        ...
        this.leaderColor = r.color || '#ec4899';
    ...
    // X-ray: fade the liver & stomach so the pancreas behind them shows
    setReveal(on) {
        for (const [id, mesh] of this.idToMesh)
            if (FRONT_IDS.has(id)) mesh.material.opacity = on ? 0.16 : 1;
    }
    ...
    animate() {
        ...
        // the gut churns with digestion and the pancreas pulses with insulin
        if (this.activity) {
            const peri = this.activity.peristalsis;
            for (const { mesh, baseScale, phase } of this.periParts) {
                const pulse = 1 + peri * 0.045 * Math.sin(this._tAcc * 3 - phase);
                mesh.scale.copy(baseScale).multiplyScalar(pulse);
            }
            if (this.pancreas) {
                const ins = this.activity.insulin;
                const pulse = 1 + ins * 0.06 * (0.5 + 0.5 * Math.sin(this._tAcc * 5));
                this.pancreas.mesh.scale.copy(this.pancreas.baseScale).multiplyScalar(pulse);
            }
        }
        this._updateLeader();
...
document.addEventListener('DOMContentLoaded', () => {
    const app = new NavadaGluco();
    app.init();
    window.navadaGluco = app;
});
