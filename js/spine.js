/**
 * NAVADA SpineAtlas — interactive segmented vertebral column.
 *
 * A static-anatomy organ page (no telemetry/scenarios — a spine doesn't cycle).
 * Reuses the platform's shared modules: scene, camera, lighting, raycasting and
 * radial explode. Each of the 16 bones is individually colour-coded, clickable
 * and labelled, with an X-ray that fades the cranial vault to reveal the bones
 * deep inside (sphenoid, ethmoid, vomer, conchae).
 */
...
// the outer vault bones — X-ray fades these so the deep facial/base bones show
const VAULT_IDS = new Set(['Cervical vertebrae', 'Thoracic vertebrae', 'Lumbar vertebrae']);
...
class NavadaSpine {
    ...
    async init() {
        try {
            ...
            this.animate();
            console.log('NAVADA SpineAtlas ready —', this.meshes.length, 'meshes /', this.idToMeshes.size, 'parts');
        } catch (err) {
            console.error(err);
    ...
    // X-ray: fade the cranial vault so the bones deep inside the face & base show
    setReveal(on) {
...
document.addEventListener('DOMContentLoaded', () => {
    const app = new NavadaSpine();
    app.init();
    window.navadaSpine = app;
});
