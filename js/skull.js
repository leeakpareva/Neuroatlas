/**
 * NAVADA SkullAtlas — interactive segmented human skull.
 *
 * A static-anatomy organ page (no telemetry/scenarios — a skull doesn't cycle).
...
const VAULT_IDS = new Set(['Frontal bone', 'Parietal bone', 'Temporal bone', 'Occipital bone']);
...
class NavadaSkull {
    ...
    async init() {
        try {
            ...
            this.animate();
            console.log('NAVADA SkullAtlas ready —', this.meshes.length, 'meshes /', this.idToMeshes.size, 'parts');
        } catch (err) {
            console.error(err);
...
document.addEventListener('DOMContentLoaded', () => {
    const app = new NavadaSkull();
    app.init();
    window.navadaSkull = app;
});
