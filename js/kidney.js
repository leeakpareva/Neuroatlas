/**
 * NAVADA RenalAtlas — interactive segmented kidney & urinary system.
 *
 * A static-anatomy organ page (no telemetry/scenarios — a kidney doesn't cycle).
 * Reuses the platform's shared modules: scene, camera, lighting, raycasting and
 * radial explode. Each part is individually colour-coded, clickable and labelled,
 * with an X-ray that fades the kidney body to reveal the renal pelvis inside.
 */
...
// the outer body — X-ray fades this so the renal pelvis inside shows
const VAULT_IDS = new Set(['Kidney']);
...
class NavadaKidney {
    ...
    async init() {
        try {
            ...
            this.animate();
            console.log('NAVADA RenalAtlas ready —', this.meshes.length, 'meshes /', this.idToMeshes.size, 'parts');
        } catch (err) {
            console.error(err);
    ...
    // X-ray: fade the kidney body so the renal pelvis inside shows
    setReveal(on) {
...
document.addEventListener('DOMContentLoaded', () => {
    const app = new NavadaKidney();
    app.init();
    window.navadaKidney = app;
});
