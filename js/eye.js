/**
 * NAVADA OptosAtlas — interactive segmented eye.
 *
 * Fourth organ in the NAVADA Atlas. Reuses the shared platform modules and adds
 * eye-specific managers: visual-response heatmap + live ocular behaviour
 * (EyeActivity — pupil, accommodation, gaze) and a pupillometry / muscle-tone
 * telemetry dashboard (EyeTelemetry). The eyeball rotates with the live gaze.
 */
...
import { EyeActivity, SCENARIOS } from './modules/EyeActivity.js';
import { EyeTelemetry } from './modules/EyeTelemetry.js';
import { TutorManager } from './modules/TutorManager.js';

const GLOBE_IDS = new Set(['Anterior segment of eyeball', 'Posterior segment of eyeball']);
const GAZE_ANGLE = 0.36;
...
class NavadaEye {
    ...
    async init() {
        try {
            this.regions = await fetch('data/eye-regions.json').then(r => r.json());
            for (const key in this.regions) this.normToKey.set(norm(key), key);

            this.eyeModel = new OrganModel(this.sceneManager.scene);
            await this.eyeModel.load('models/eye.glb');
            ...
            this.activity = new EyeActivity(this.sceneManager.scene, this.idToMesh, this.regions);

            this.telemetry = new EyeTelemetry(
                ...
                { status: document.getElementById('tele-status'),
                  pupil: document.getElementById('tele-pupil'),
                  light: document.getElementById('tele-light'),
                  focus: document.getElementById('tele-focus') },
                this.activity);
            ...
            this.animate();
            console.log('NAVADA OptosAtlas ready —', this.meshes.length, 'parts');
        } catch (err) {
            console.error(err);
    ...
    _prepareMeshes() {
        this.eyeModel.model.traverse((o) => {
            if (!o.isMesh) return;
    ...
    _addHelpers() {
        ...
        this.grid = new THREE.GridHelper(7, 14, 0xa78bfa, 0x2a2440);
    ...
    _frameModel() {
        // same 3/4 framing as the other atlases
        const sphere = this.eyeModel.getBoundingSphere();
        const dist = this.eyeModel.getOptimalCameraDistance(this.sceneManager.camera.fov);
        this.camera.camera.position.set(dist * 0.55, dist * 0.22, dist * 0.82);
        this.camera.setTarget(this.eyeModel.getCenter());
    ...
    select(id) {
        ...
        this.leaderColor = r.color || '#a78bfa';
    ...
    // X-ray: fade the globe (segments) so the iris & lens inside show
    setReveal(on) {
        for (const [id, mesh] of this.idToMesh)
            if (GLOBE_IDS.has(id)) mesh.material.opacity = on ? 0.16 : 1;
    }
    ...
    animate() {
        ...
        // the eyeball looks around with the live gaze
        if (this.eyeModel && this.activity) {
            this.eyeModel.model.rotation.y = this.activity.gaze.x * GAZE_ANGLE;
            this.eyeModel.model.rotation.x = -this.activity.gaze.y * GAZE_ANGLE;
        }
        this._updateLeader();
...
document.addEventListener('DOMContentLoaded', () => {
    const app = new NavadaEye();
    app.init();
    window.navadaEye = app;
});
